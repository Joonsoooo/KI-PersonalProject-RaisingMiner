...
void GoldMine::initializeMine()
{
    ...
    for (int y = 0; y < HEIGHT; ++y) // 높이 = 행 
    {
        for (int x = 0; x < WIDTH; ++x) // 너비 = 열
        {
            ...
            if (randVal < 0.30f)
            {
                grid[y][x] = MineralType::ROCK; // 일반 돌 30%
            }
            else if (randVal < 0.40f)
            {
                grid[y][x] = MineralType::COAL; // 석탄 10%
            }
            ...
            {
                grid[y][x] = MineralType::GOLD; // 금 30%
            }
            else {
                grid[y][x] = MineralType::DIAMOND; // 다이아몬드 10%
            }
        }
    }
