...
void DiamondMine::initializeMine()
{
    ...
    for (int y = 0; y < HEIGHT; ++y) // 높이 = 행 
    {
        for (int x = 0; x < WIDTH; ++x) // 너비 = 열
        {
            ...
            if (randVal < 0.20f)
            {
                grid[y][x] = MineralType::ROCK; // 일반 돌 20%
            }
            else if (randVal < 0.40f)
            {
                grid[y][x] = MineralType::COAL; // 석탄 20%
            }
            else if (randVal < 0.60f)
            {
                grid[y][x] = MineralType::IRON; // 철 20%
            }
            else if (randVal < 0.80f)
            {
                grid[y][x] = MineralType::GOLD; // 금 20%
            }
            else {
                grid[y][x] = MineralType::DIAMOND; // 다이아몬드 20%
            }
        }
    }
